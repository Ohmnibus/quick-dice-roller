...
import ohm.library.widget.PageControl.OnPageControlClickListener;
...
public class SwipeView extends HorizontalScrollView {
	...
	private static final int DEFAULT_BORDER_SWIPE_AREA = 64; //dp. 0 for full area.
	private static int DEFAULT_SWIPE_THRESHOLD = 60;
	
	private LinearLayout mLinearLayout;
	private Context mContext;
	private int SCREEN_WIDTH;
	private int mBorderWsipeArea;
	...
	private boolean mNotStartedInValidArea = false;
...
//		SCREEN_WIDTH = (int) (display.getWidth());
		...
		//SCREEN_WIDTH = (int)((double)config.screenWidthDp * metrics.density + .5); //More accurate
		
		mBorderWsipeArea = (int)((double)DEFAULT_BORDER_SWIPE_AREA * metrics.density + .5);
	...
	public void setPageControl(PageControl pageControl) {
		...
		pageControl
				.setOnPageControlClickListener(new OnPageControlClickListener() {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (ev.getAction() == MotionEvent.ACTION_DOWN) {
			...
			mNotStartedInValidArea = mBorderWsipeArea > 0
					&& mMotionStartX > mBorderWsipeArea
					&& mMotionStartX < (SCREEN_WIDTH - mBorderWsipeArea);
			//Log.d("onInterceptTouchEvent", "mNotStartedInValidArea: " + mNotStartedInValidArea);
		} else if (ev.getAction() == MotionEvent.ACTION_MOVE) {
			detectMostlyScrollingDirection(ev);
		}

		if (mNotStartedInValidArea) {
	...
	private class SwipeOnTouchListener implements View.OnTouchListener {
		...
		public boolean onTouch(View v, MotionEvent event) {
			if(mOnTouchListener!=null 
					&& !mJustInterceptedAndIgnored 
					|| mOnTouchListener!=null 
					&& mSendingDummyMotionEvent) { //send on touch event to onTouchListener set by an application implementing a SwipeView and setting their own onTouchListener
